...
export async function importBatch(env: ImportEnv, userId: string, batch: ImportBatch): Promise<ImportResult> {
  ...
  for (let offset = 0; offset < highlights.length; offset += 50) {
    ...
    // Keep JSON rows outermost: reordering these joins scans the entire user library
    // once per incoming highlight. CROSS JOIN preserves indexed lookups by highlight ID.
